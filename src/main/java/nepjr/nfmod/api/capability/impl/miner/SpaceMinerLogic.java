package nepjr.nfmod.api.capability.impl.miner;
...
import nepjr.nfmod.api.capability.ISpaceMiner;
...
public class SpaceMinerLogic
{
    ...
    protected final ISpaceMiner miner;
     ...
     */
    public SpaceMinerLogic(@NotNull MetaTileEntity metaTileEntity, int fortune, int speed, int maximumRadius,
                      ICubeRenderer pipeTexture) {
        this.metaTileEntity = metaTileEntity;
        this.miner = (ISpaceMiner) metaTileEntity;
    ...
    public void performMining() {
        ...
        if (blocksToMine.isEmpty()) {
            ...
            if(blocksToMine.getFirst() == blocksToMine.getLast()) // If the first in the list is also the last, reset
            {
                this.resetArea();
            }
            if (blocksToMine.isEmpty()) {
                this.resetArea(); // do it  again
            }
        }
    }
    ...
    private void mineAndInsertItems(NonNullList<ItemStack> blockDrops, WorldServer world) {
        ...
        if (GTTransferUtils.addItemsToItemHandler(metaTileEntity.getExportItems(), true, blockDrops)) {
            ...
            // Normally some code here exists to replace the block, however we don't want that
            System.out.println("Mining " + world.getBlockState(blocksToMine.getFirst()));
            mineX.set(blocksToMine.getFirst().getX());
            mineZ.set(blocksToMine.getFirst().getZ());
            mineY.set(blocksToMine.getFirst().getY());
            blocksToMine.removeFirst(); // Remove it from the queue to mine the next block
            onMineOperation();
    ...
    public void renderPipe(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        ...
        for (int i = 0; i < this.pipeLength; i++) {
            ...
            getPipeTexture().render(renderState, translation, pipeline, ISpaceMiner.PIPE_CUBOID);
        }
    }
